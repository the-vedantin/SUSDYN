...
def generate_report(output_path: str, data: dict,
    ...
    # ── Topology (single-model): name the suspension flavour per axle and
    # list the rates of the elements that car ACTUALLY has.
    topo_desc = vp.get('topology_desc')
    if topo_desc:
        rows.append(('Suspension Topology', str(topo_desc), '—'))
    for suffix, tag in (('front', 'Front'), ('rear', 'Rear')):
        mode = vp.get(f'topology_mode_{suffix}', 'standard')
        if mode == 'decoupled':
            kh = vp.get(f'decoupled_heave_rate_{suffix}_Npm', 0.0)
            kr = vp.get(f'decoupled_roll_rate_{suffix}_Npm', 0.0)
            mh = vp.get(f'decoupled_heave_MR_{suffix}', 0.0)
            mr_ = vp.get(f'decoupled_roll_MR_{suffix}', 0.0)
            rows += [
                (f'{tag} Heave Spring Rate (decoupled)', f'{kh:.0f}', 'N/m'),
                (f'{tag} Roll Spring Rate (decoupled)',  f'{kr:.0f}', 'N/m'),
                (f'{tag} Heave / Roll MR (geometric)',
                 f'{mh:.3f} / {mr_:.3f}', '—'),
            ]
        elif mode == 'heave_tbar':
            k3 = vp.get(f'heave_3rd_rate_{suffix}_Npm', 0.0)
            m3 = vp.get(f'heave_3rd_MR_{suffix}', 0.0)
            rows += [
                (f'{tag} 3rd (Heave) Spring Rate', f'{k3:.0f}', 'N/m'),
                (f'{tag} 3rd Spring MR (geometric)', f'{m3:.3f}', '—'),
            ]
