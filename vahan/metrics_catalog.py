...
CATALOG = [
    ...
    # RC LATERAL position from the same left/right IC intersection that
    # gives rc_height (filled in _run_sweep post-processing).  Migration
    # of the RC sideways in roll is the classic geometry red flag a
    # height-only plot hides.
    dict(key='rc_lateral',     label='Roll Centre Lateral Pos', unit='mm',  category='Geometry',  fn=_roll_axis_incl_placeholder, scope='axle'),
    ...
    # \u2500\u2500 Topology-specific elements (single-model: each suspension flavour
    #    exposes the metrics of ITS actual mechanism).  Values are filled by
    #    main_window's sweep post-processing from the topology solver
    #    (TwinRockerDecoupledSolver / HeaveTBarRockerSolver) \u2014 the per-corner
    #    fn is a placeholder, like roll_axis_incl.
    #    'topologies': which flavour(s) this metric physically exists on:
    #        'decoupled'  \u2014 twin-bellcrank cradle (heave + roll coilovers)
    #        'heave_tbar' \u2014 central T-bar WITH the 3rd (heave) spring
    #        'tbar'       \u2014 central T-bar WITHOUT the 3rd spring (plain ARB)
    #    Entries WITHOUT a 'topologies' field are generic (always shown).
    dict(key='heave_spring_mr',  label='Heave-spring MR',            unit='-',  category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'decoupled'}),
    dict(key='roll_spring_mr',   label='Roll-spring MR',             unit='-',  category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'decoupled'}),
    dict(key='heave_spring_len', label='Heave-spring Length',        unit='mm', category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'decoupled'}),
    dict(key='roll_spring_len',  label='Roll-spring Length',         unit='mm', category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'decoupled'}),
    dict(key='third_spring_len', label='3rd (Heave) Spring Length',  unit='mm', category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'heave_tbar'}),
    dict(key='third_spring_mr',  label='3rd (Heave) Spring MR',      unit='-',  category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'heave_tbar'}),
    dict(key='tbar_twist',       label='T-bar Twist',                unit='\u00b0',  category='Topology', fn=_roll_axis_incl_placeholder, scope='axle',
         topologies={'heave_tbar', 'tbar'}),
    dict(key='coil_len',         label='Corner Coil Length (bellcrank)', unit='mm', category='Topology', fn=_roll_axis_incl_placeholder, scope='corner',
         topologies={'heave_tbar', 'tbar'}),
]
