...
const HP_LIST = [
    ...
    // ── Topology-specific hardware ──────────────────────────────────────────
    // These are emitted per-corner by Vahan's "Copy for Onshape" only when the
    // chosen topology actually has them; on topologies that lack them the
    // exporter sends 0,0,0 and the absent-point guard skips them (no ghosts).
    // They are PASTE-ONLY (no manual coordinate params below), so the manual
    // branch + editing logic guard against their missing parameters.
    "damper_chassis_pt", "damper_outer_pt",   // DIRECT-acting damper line
    "arb_lca_attach",                         // CONTROL_ARM ARB lever -> LCA
    "tbar_arm_end", "tbar_base_chassis",      // T-bar 3rd-element nodes
    "tbar_drop_top", "tbar_top_node",
    "rocker_tbar_drop_pt"
];
        ...
        for (var ci = 0; ci < size(CORNERS); ci += 1)
        {
            ...
            for (var hi = 0; hi < size(HP_LIST); hi += 1)
            {
                ...
                if (hasPaste)
                {
                    ...
                    // Skip points the exporter emitted as 0,0,0 -- these are
                    // hardpoints ABSENT from this corner's topology (e.g. a
                    // direct-acting axle has no pushrod/rocker, a no-ARB axle
                    // has no ARB points).  A real hardpoint never sits exactly
                    // at the car origin, so 0,0,0 unambiguously means "absent".
                    // Without this guard, mixed-topology exports spawn origin
                    // ghost points (matches the manual-mode guard below).
                ...
                {
                    // Manual mode: read individual coordinate params.  The
                    // topology-specific hardware names (damper_*, tbar_*,
                    // arb_lca_attach, rocker_tbar_drop_pt) are PASTE-ONLY and
                    // have no isLength params, so guard each lookup with
                    // try-silent and skip the point if the param is absent —
                    // otherwise dereferencing an undefined parameter errors.
                    var px = try silent(definition[prefix ~ "_x"]);
                    var py = try silent(definition[prefix ~ "_y"]);
                    var pz = try silent(definition[prefix ~ "_z"]);
        ...
        for (var corner in CORNERS)
        {
            for (var hp in HP_LIST)
            {
                ...
                if (pt == undefined)
                    continue;
                var prefix = corner ~ "_" ~ hp;
                // Only populate manual fields that actually exist.  The
                // topology-specific hardware names are paste-only (no isLength
                // params), so a declared param is present in oldDefinition;
                // an undeclared one reads undefined and we skip it (otherwise
                // we would inject an unused key into the definition map).
                if (try silent(oldDefinition[prefix ~ "_x"]) == undefined)
                    continue;
